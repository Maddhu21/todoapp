                                        ...
                                        @if ($statusFlag)
                                            <li>
                                                <a class="dropdown-item"
                                                    href="{{ route('task.status.update', [$item->id, $status->firstWhere('name', 'Complete')?->id]) }}">Mark
                                                    as Done</a>
                                            </li>
                                        @else
                                            <li>
                                                <a class="dropdown-item"
                                                    href="{{ route('task.status.update', [$item->id, $status->firstWhere('name', 'Incomplete')?->id]) }}">Mark
                                                    as Incomplete</a>
                                            </li>
                                        @endif

                                        <li><a href="#" class="dropdown-item btn-edit-task"
    ...
    <div class="modal fade" id="addTaskModal" tabindex="-1" aria-labelledby="addTaskModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow">
                ...
                <form id="addTaskForm" action="{{ route('task.add.post') }}" method="POST">
                    @csrf
            ...
            $('#addTaskForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            toastr.success('Task has been successfully created!', 'Success')
                            setTimeout(() => {
                                location.reload();
                            }, 1000); // waits 2 seconds

                        } else {
                            toastr.error('Task not created.', 'Fail');
                            setTimeout(() => {
                                location.reload();
                            }, 1000); // waits 2 seconds
                        }
