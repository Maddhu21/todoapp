                ...
                <form id="tableSelectForm">
                    <select name="table_names" id="tableSelect" class="form-select mb-3">
                        ...
                        @foreach ($masterTables as $tableItem)
                            <option value="{{ $tableItem }}">
                                {{ ucwords(str_replace($identifier, $replacement, $tableItem)) }}</option>
                        @endforeach
                    </select>
                </form>
    ...
    <script type="module">
        ...
        function fetchData(page = 1) {
            ...
            $.post("{{ route('masters.data') }}", {
                ...
                if (res.data.length === 0) {
                    $('#tableBody').append('<tr><td colspan="' + (currentColumns.length + 2) +
                        '" class="text-center">No data found</td></tr>');
                } else {
        ...
        function renderPagination(res) {
            ...
            for (let i = 1; i <= res.last_page; i++) {
                $('#pagination').append(
                    `<li class="page-item ${i === res.current_page ? 'active' : ''}"><a class="page-link" href="#">${i}</a></li>`
                );
            }
        }
